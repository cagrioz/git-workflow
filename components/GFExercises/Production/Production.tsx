...
const supportedCommands = ["commit", "checkout", "merge", "branch", "tag"];
...
function Production({ active, updateScore, reset }: { active: boolean; updateScore: any; reset: boolean }) {
    // Success snackbar
    ...
    function addCommand() {
        ...
        if (currentStep === 0) {
            ...
            // Check if the command is correct
            if (command == "git merge feature-1") {
                openSuccessSnackbar("Correct command! You can now continue to the next exercise");
            } else {
                openErrorSnackbar("Command not correct");
                return;
            ...
            // Check if the command is correct
            if (command == "git checkout release") {
                openSuccessSnackbar("Correct command! You can now continue to the next exercise");
            } else {
                openErrorSnackbar("Command not correct");
                return;
    ...
    return (
        <>
            {!disabled && (
                <ProgressBar completed={Math.round(12.5 * currentStep)} maxCompleted={Math.round(8 * 12.5)} />
            )}
            <Gitgraph options={gitGraphOptions as GitgraphOptions} key={executedCommands.length}>
                {(gitgraph) => {
                    ...
                    let releaseBranch = master.branch("release").commit("Initial commit");

                    let newBranch = releaseBranch.branch("develop").commit("Initial commit");

                    let featureBranch = newBranch.branch("feature-1").commit("Initial commit");

                    executedCommands.forEach((command) => {
                        const message = command.split(" ");

                        if (message[1] === "commit" || message[1] === "tag") {
                            // Commit to the current branch that is active
                            gitgraph.commit(message[2]);

                            if (message[1] === "tag") {
                                master.tag(message[2]);
                            }
                        } else if (
                            ...
                            if (message[2] === "develop") {
                                releaseBranch.merge(newBranch);
                            }

                            if (message[2] === "feature-1") {
                                newBranch.merge(featureBranch);
                            }

                            if (message[2] === "master") {
                                master.merge(releaseBranch);
                            }
                        }
                    });
                }}
